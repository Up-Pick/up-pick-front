...
export const creditApi = {
  ...
  getCredit: async (): Promise<CreditResponse> => {
    const response = await axiosInstance.get<ApiResponse<CreditResponse>>(
      '/main/api/v1/members/me/credit'
    );
  ...
  chargeCredit: async (data: CreditChargeRequest): Promise<CreditChargeResponse> => {
    const response = await axiosInstance.post<ApiResponse<CreditChargeResponse>>(
